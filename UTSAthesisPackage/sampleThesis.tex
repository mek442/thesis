...
Our vision is to provide a framework to determine when mock and when not. In the figure \ref{figure:vision}, we can see the smart mocking system takes performance test case, code commit and regression test suite as the input of the framework and output is ranked list of test cases based on performance impact and behavioral incompatibility. There are two major component of the system, one is performance impact analysis component and the other is behavioral incompatibility analysis component. Main objective of these two component are not only to provide recommendation when to mock, when not and when to replace mock by real code, but also rank the regression test cases.
